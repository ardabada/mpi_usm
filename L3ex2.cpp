/*
передача сообщения против часовой стрелке по нечетным процессам, начиная от процесса incep
*/
#include <mpi.h>
#include <stdio.h>
#include <stdlib.h>
...
int getRecvRank(int current) {
	...
	if ((result > size && result % size > incep && current > incep) || (current < incep && result > incep))
		result = incep;
	result %= size;
	return result; 
}
...
int main(int argc, char** argv) {
	...
	//работают только нечетные процессы и процесс incep, поэтому вывод только у них
