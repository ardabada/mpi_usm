...
int main(int argc, char** argv) {
	...
	MPI_Scatter(source_matrix, size, MPI_INT, rows, size, MPI_INT, 0, MPI_COMM_WORLD);
	
    printf("MPI_Scatter (rank %d): ", rank);
    for (int i = 0; i < size; ++i)
        printf("%3d ", rows[i]);
    printf("\n");
    MPI_Barrier(MPI_COMM_WORLD);
	
	MPI_Send(rows, size, MPI_INT, 0, 10, MPI_COMM_WORLD);
	...
	if (rank == 0) {
		...
		for (int i = 0; i < size; i++) {
			MPI_Recv(&test_matrix[i * size], size, MPI_INT, i, 10, MPI_COMM_WORLD, &status); 
		}
		printf("\nMPI_Recv:\n");
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++)
                printf("%5d", test_matrix[i * size + j]);
            printf("\n");
        }
        printf("\n");
    }
